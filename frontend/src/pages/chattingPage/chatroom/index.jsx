...
export default function ChatRoom({
  props: { socket, room, setChatRooms, user, chatRooms, setSelectedRoom },
}) {
  ...
  const updateChatList = useCallback(
    (data, roomId) => {
      setChatRooms((chatRooms) => {
        const targetRoom = chatRooms.filter(
          (chatRoom) => chatRoom.dmRoomId === data.roomId
        )[0];
        let restRooms = [];

        if (targetRoom.dmRoomId) {
          restRooms = chatRooms.filter(
            (chatRoom) => chatRoom.dmRoomId !== data.roomId
          );
  ...
  const responseChat = useCallback(
    (data) => {
      if (!data.chats) {
        return;
      }
  ...
  const receiverNotExist = useCallback(
    (data) => {
  ...
  const responseVideo = useCallback(
    (data) => {
      if (!videoTextRef.current) {
        return;
      }
      ...
      setSelectedRoom(
        chatRooms.filter((chatRoom) => chatRoom.dmRoomId === data.roomId)[0]
      );
  ...
  const responseVideoAccept = useCallback(
    (data) => {
      setVideoClicked(false);
      setVideoStart(true);
    },
  ...
  useEffect(() => {
    ...
    socket.emit("joinRoom", {
      ...
      receiver:
        room.dmRoomId.split("_")[
          1 - room.dmRoomId.split("_").indexOf(user.memberMail)
        ],
    });
      ...
      <S.SectionChat>
        <S.HeaderChat>
          ...
          <S.HeaderButtonNormal
            onClick={onClickVideoRequest}
            visible={!videoStart && room.dmRoomId}
          >
            <AddIcCallIcon />
          </S.HeaderButtonNormal>
          ...
          <S.VideoModal>
            ...
            <S.VideoButtonWrapper>
              {videoTextRef &&
                ...
                !videoTextRef.current.innerText.includes("에게") && (
                  <S.VideoConfirmButton onClick={onClickVideoConfirm}>
                    confirm
                  </S.VideoConfirmButton>
                )}
              <S.VideoRejectButton onClick={onClickVideoRejcet}>
                reject
              </S.VideoRejectButton>
            </S.VideoButtonWrapper>
          </S.VideoModal>
        ...
        <S.DictionaryWrapper>
          {room.dmRoomId && (
            <S.VideoWrapper>
              <Video
                props={{ videoStart, setVideoStart, room, user, socket }}
              />
              <S.ListChat>
                {chats.map((chat, index) => {
                  ...
                  if (index !== 0) {
                    ...
                    if (isCreated.getDate() !== prevCreated.getDate()) {
                      displayDate = true;
                      today = `${
                        isCreated.getMonth() + 1
                      }월 ${isCreated.getDate()}일 ${getDay(isCreated)}요일`;
                    }
                  }
                  ...
                  if (index !== 0) {
                    ...
                    const prevCreatedDate = new Date(
                      chats[index - 1].createdAt
                    );
                    prevCreatedDate.setHours(prevCreatedDate.getHours());

                    if (
                      prevSender !== chat.memberName ||
                  ...
                  const Translate = async () => {
                    const response = await fetch(
                      process.env.REACT_APP_TRANSLATION_SERVER +
                        "/ko/en/" +
                        chat.messageContent
                    );
                    ...
                    setChats((chats) =>
                      chats.map((c) => {
                        if (
                          c.memberName === chat.memberName &&
                          c.messageContent === chat.messageContent &&
                  ...
                  return (
                    <S.ChatWrapper
                      key={"" + chat.messageContent + index + chat.memberName}
                    >
                      {
                        <>
                          {displayDate && (
                            <S.Datewrapper>{today}</S.Datewrapper>
                          )}
                          {chat.memberName === user.memberName ? (
                            <S.MyChatWrapper>
                              {displayProfile && (
                                <S.MyUserName>{chat.memberName}</S.MyUserName>
                              )}
                              <S.TimeWrapper>
                                {
                                  <S.TranslateWrapper>
                                    {!chat.translate && (
                                      <S.TranslateButton onClick={Translate}>
                                        번역
                                      </S.TranslateButton>
                                    )}
                                    {displayTime && (
                                      <S.Time>{timeValue}</S.Time>
                                    )}
                                  </S.TranslateWrapper>
                                }
