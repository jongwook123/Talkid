...
export default function TeacherMatchPage() {
    ...
    // 내 빈 일정
    const [mySchedules, setMySchedules] = useState(Array(32).fill(0).map((_) => []));

    // 모든 빈 일정
    const [schedules, setSchedules] = useState(Array(32).fill(0).map((_) => []));

    // 내 일정
    const [meetings, setMeetings] = useState(Array(32).fill(0).map((_) => []));
        ...
        const fetchData = async () => {
            try {
                ...
                const startTimes1 = meetingSchedules.map(schedule => {
                    const startTime = schedule.meetingScheduleStart.split('T')[0].slice(8, 10);
                    return startTime;
                });

                const mySchedulesCopy = Array(32).fill(0).map((_) => []);
                const schedulesCopy = Array(32).fill(0).map((_) => []);
                ...
                const meetingsCopy = Array(32).fill(0).map((_) => []);
        ...
        <>
            <S.MatchSection>
                <S.CalendarSection>
                    ...
                    <Body totalDate={totalDate} today={today} month={month} year={year} mySchedules={mySchedules} schedules={schedules} meetings={meetings} setClickedData={setClickedData} />
                </S.CalendarSection>
                <S.ListSection>
