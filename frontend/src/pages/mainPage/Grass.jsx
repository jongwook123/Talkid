...
export default function Grass({
  options = { bW: 0.2, bH: 1.5, joints: 1 },
  width = 300,
  instances = 2000000,
  ...props
}) {
  ...
  const baseGeom = useMemo(() => {
    const geometry = new THREE.PlaneGeometry(bW, bH, 1, joints).translate(
      0,
      bH / 2,
      0
    );
    return new THREE.BufferGeometry().copy(geometry);
  }, [bW, bH, joints]);
  ...
  const groundGeo = useMemo(() => {
    ...
    // Update the vertex positions of the buffer geometry based on the getYPosition function
    for (let i = 0; i < vertexCount; i++) {
      const x = positions[i * 3];
      const z = positions[i * 3 + 2];
      positions[i * 3 + 1] = getYPosition(x, z) - 1;
    }

    // Compute vertex normals after updating the vertex positions.
